...
- $P(C_k | x) = \frac{P(C_k | x)}{P(x)} = \frac{P(x | C_k}{\sum_{k} P(C_k, x)} = \frac{P(x|C_k)P(C_k)}{P(x)}$
- $P(C_k | x)$를 최대화시키는 $k$를 구하는 것이 좋은 결정

- 이진분류의 경우
- 결정영역: $\mathcal{R}_i = \{x : $pred$(x) = C_i\}$
- 분류오류 확률: $P(mis) = P(x \in R_1, C_2) + P(x \in R_2, C_1) = \int_{\mathcal{R}_1}P(x, C_2)dx + \int_{\mathcal{R}_2}P(x, C_1)dx$
...
- $P($correct$) = \displaystyle\sum_{k = 1}^{K}P(x \in \mathcal{R}_k, C_k) = \displaystyle\sum_{k = 1}^{K}\int_{\mathcal{R}_k}P(x, C_k)dx$
...
- $\mathbb{E}(L) = \displaystyle\sum_{k}\displaystyle\sum_{j}\int_{\mathcal{R}_j}L_{kj}P(x, C_k)dx$
- 기대손실을 최소화 시켜야 우수한 분류를 할 수 있음

- $\mathbb{E}(L) = \displaystyle\sum_{k}\displaystyle\sum_{j}\int_{\mathcal{R}_j}L_{kj}P(x, C_k)dx =  \int_{\mathcal{X}}\left(\displaystyle\sum_{k = 1}^{K}L_{k\hat{C}(x)}P(C_k | x)\right) p(x)dx$ 로 표현이 가능
...
- $\hat{C}(x) = argmin\displaystyle\sum_{k = 1}^{K}L_{kj}P(C_k | x) = \displaystyle\sum_{k = 1}^{K}P(C_k | x) - P(C_j | x) = 1 - P(C_j | x)$
- 따라서 기대손실을 최소화 시키는 것은 범함수 $\hat{C}(x)$를 최대화 시키는 것과 동일
- $\hat{C}(x) = argmin\{1 - P(C_j | x)\} \rightarrow argmaxP(C_j | x)$

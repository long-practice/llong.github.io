...
- $P(C_k \mid x) = \frac{P(C_k \mid x)}{P(x)} = \frac{P(x \mid C_k}{\sum_{k} P(C_k, x)} = \frac{P(x \mid C_k)P(C_k)}{P(x)}$
- $P(C_k \mid x) $ 를 최대화시키는 $k$를 구하는 것이 좋은 결정

- 이진분류의 경우
- 결정영역: $\mathcal{R}_i = \{x : \mbox{pred} (x) = C_i\} $
- 분류오류 확률: $ \displaystyle P(mis) = P(x \in \mathcal{R}_1, C_2) + P(x \in \mathcal{R}_2, C_1) = \int_{\mathcal{R}_1} P(x, C_2) \dx + \int_{\mathcal{R}_2}P(x, C_1) \dx$

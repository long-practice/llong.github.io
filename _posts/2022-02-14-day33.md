...
- 이번에는 우도를 최대화 하는 공분산행렬 $\Sigma_{ML}$을 구해보면<br>
  ...
  $\displaystyle l(\Lambda)=\frac{N}{2} \ln |\Lambda|-\frac{1}{2} \sum_{n=1}^{N} \operatorname{tr}((\mathbf{x}\_{n}-\boldsymbol{\mu})(\mathbf{x}\_{n}-\boldsymbol{\mu})^{T} \Lambda\right)=\frac{N}{2} \ln |\Lambda|-\frac{1}{2} \operatorname{tr}(\mathbf{S} \Lambda)$<br>

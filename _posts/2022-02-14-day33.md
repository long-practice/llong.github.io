...
$\mathcal{N}(\mathbf{x} \mid \boldsymbol{\mu}, \Sigma)=\displaystyle\frac{1}{(2 \pi)^{D / 2}} \frac{1}{\vert\Sigma\vert^{1 / 2}} \exp \bigg{-\frac{1}{2}(\mathbf{x}-\boldsymbol{\mu})^{T} \Sigma^{-1}(\mathbf{x}-\boldsymbol{\mu})\bigg\}$<br>
<br>
...
- $\boldsymbol{y}$의 확률밀도함수<br>
  $\displaystyle p(\mathbf{y})=p(\mathbf{x}) \left\vert \mathbf{J} \right\vert = \prod_{j=1}^{D} \frac{1}{(2 \pi \lambda_{j})^{1 / 2}} \exp \bigg\{-\frac{y_{j}^{2}}{2 \lambda_{j}} \bigg\}$<br>
- 위의 식은 서로 다른 $D$개의 정규분포가 독립적으로 곱해진 것이고, 변환된 식 또한 서로 다른 정규분포가 독립적으로 곱해진 것<br>
<br>
- $\boldsymbol{y}$의 정규화<br>
  $\displaystyle \int p(\mathbf{y}) \mathrm{d} \mathbf{y}=\prod_{j=1}^{D} \int_{-\infty}^{\infty} \frac{1}{(2 \pi \lambda_{j})^{1 / 2}} \exp \bigg\{-\frac{y_{j}^{2}}{2 \lambda_{j}}\bigg\} \mathrm{d} y_{j}=1$<br>
...
- 다변량(multivariate) 확률변수의 기댓값<br>
  ...
  <br>
- 1차 적률 구하기<br>
  $\displaystyle \mathbb{E} [ \mathbf{x} ]= \frac{1}{(2 \pi)^{D / 2} \left\vert \Sigma\right\vert^{1 / 2}} \int \exp \bigg\{-\frac{1}{2}(\mathbf{x}-\boldsymbol{\mu})^{T} \boldsymbol{\Sigma}^{-1}(\mathbf{x}-\boldsymbol{\mu})\bigg\} \mathbf{x} d \mathbf{x}$
  $\displaystyle = \frac{1}{(2 \pi)^{D / 2}\left\vert\Sigma\right\vert^{1 / 2}} \int \exp \bigg\{-\frac{1}{2}(\mathbf{z})^{T} \boldsymbol{\Sigma}^{-1}(\mathbf{z})\bigg\}(\mathbf{z}+\boldsymbol{\mu}) d \mathbf{z}$<br>
...
#### 공분산
- 2차 적률 구하기<br>
  $\displaystyle E[\mathbf{x x}^{T}]=\frac{1}{(2 \pi)^{D / 2}\left\vert\Sigma\right\vert^{1 / 2}} \int \exp \bigl\{-\frac{1}{2}(\mathbf{x}-\boldsymbol{\mu})^{T} \boldsymbol{\Sigma}^{-1}(\mathbf{x}-\boldsymbol{\mu})\bigr\} \mathbf{x x}^{T} d \mathbf{x}$<br>

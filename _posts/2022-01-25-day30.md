...
<br>
$ \int_{-\infty}^{\infty} \mathcal{N}(x|\mu, \sigma ^2)dx = 1 \rightarrow \frac{d}{dy}\int_{-\infty}^{\infty} \mathcal{N}(x|\mu, y)dx = 0$로 해결가능<br>
<br>
...
<br>
$\frac{\partial}{\partial \mu}\ln p(X|\mu, \sigma^2) = 0$ 를 만족시키는 $\mu$값을 구하게되면 최대우도를 갖게하는 $\mu$의 값을 구할 수 있음<br>
<br>

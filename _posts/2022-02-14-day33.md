...
- 주변확률분포를 계산하는 전략은 다음과 같음<br>
  $\begin{aligned} p(\mathbf{x}\_{a}) &=\int p(\mathbf{x}\_{a}, \mathbf{x}\_{b}) \mathrm{d} \mathbf{x}\_{b} \\\\\\ &=\int \alpha \exp \\{-\frac{1}{2}(\mathbf{x}-\boldsymbol{\mu})^{T} \Sigma^{-1}(\mathbf{x}-\boldsymbol{\mu})\\} \mathrm{d} \mathbf{x}\_{b} \\\\\\ &=\int \alpha \exp \\{f(\mathbf{x}\_{b}, \mathbf{x}\_{a})+g(\mathbf{x}\_{a})+\text{ const }\\} \mathrm{d} \mathbf{x}\_{b} \\\\\\ &=\int \alpha \exp \\{f(\mathbf{x}\_{b}, \mathbf{x}\_{a})-\tau+\tau+g(\mathbf{x}\_{a})+\text{ const }\\} \mathrm{d} \mathbf{x}\_{b} \\\\\\ &=\int \alpha \exp \\{f(\mathbf{x}\_{b}, \mathbf{x}\_{a})-\tau\\} \exp \\{\tau+g(\mathbf{x}\_{a})+\text{ const }\\} \mathrm{d} \mathbf{x}\_{b} \\\\\\ &=\alpha \exp \\{\tau+g(\mathbf{x}\_{a})+\text{ const }\\} \int \exp \\{f(\mathbf{x}\_{b}, \mathbf{x}\_{a})-\tau\\} \mathrm{d} \mathbf{x}\_{b} \\\\\\ &=\alpha \beta \exp \\{\tau+g(\mathbf{x}\_{a})+\text{ const }\\} \end{aligned}$<br>
<br>
...
- 적분하는 부분은 다음과 같음<br>
  $\displaystyle \int \exp \{-\frac{1}{2}(\mathbf{x}\_{b}-\Lambda_{b b}^{-1} \mathbf{m})^{T} \Lambda_{b b}(\mathbf{x}\_{b}-\Lambda_{b b}^{-1} \mathbf{m})\} d \mathbf{x}\_{b}$<br>
<br>

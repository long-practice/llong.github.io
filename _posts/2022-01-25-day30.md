...
- 오차함수: $\displaystyle E(w) = \frac{1}{2} \sum_{n=1}^{N} \left\{ y(x_n, w) - t_n \right\} ^2$
...
- $\displaystyle \tilde E(w) = \frac{1}{2} \sum_{n=1}^{N} \left\{y(x_n, w) - t_n \right\}^2 + \frac{\lambda}{2} \left\Vert w \right\Vert ^2$에서 뒤에있는 $\left\Vert w \right\Vert$가 규제항

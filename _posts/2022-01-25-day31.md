...
<br>
- 이진분류의 경우
- 결정영역: $\mathcal{R}\emph{i} = \{x : \mbox{pred} (x) = C_i \} $
- 분류오류 확률: $\displaystyleP(\mbox{mis}) = P(x \in \mathcal{R}\emph{1}, C_2) + P(x \in \mathcal{R}\emph{2}, C_1) = \int_\mathcal{R}\emph{1} P(x, C_2) dx + \displaystyle \int_{\mathcal{R}\emph{2}P(x, C_1) dx$

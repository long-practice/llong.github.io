...
<br>
  2. 가우시안 기저함수<br>
     $\displaystyle \phi_{j}(x)=\exp \left\{-\frac{\left(x-\mu_{j}\right)^{2}}{2 s^{2}}\right\}$<br>
<br>
  3. 시그모이드 기저함수<br>
...
<br>
- 양변에 로그를 취해주면<br>
  $\displaystyle \ln p(\mathbf{t} \mid \mathbf{w}, \beta)=\sum_{n=1}^{N} \ln \mathcal{N}\left(t_{n} \mid \mathbf{w}^{T} \phi\left(\mathbf{x}_{n}\right), \beta^{-1}\right)=\frac{N}{2} \ln \beta-\frac{N}{2} \ln (2 \pi)-\beta E_{D}(\mathbf{w})$<br>
  $\displaystyle E_{D}(\mathbf{w})=\frac{1}{2} \sum_{n=1}^{N}\left\{t_{n}-\mathbf{w}^{T} \phi\left(\mathbf{x}_{n}\right)\right\}^{2}$
- 따라서 로그우도함수를 최대화 시키는 $\mathbf{w}$는 $E_{D}{\mathbf{x}_n}$을 최소화시키는 값과 동일
...
- 마찬가지 방법으로 우도함수를 최대화시키는 로그우도함수를 바이어스($w_0$)와 분산($\beta$)를 구해보면 다음과 같음<br>
  $\displaystyle E_{D}(\mathbf{w})=\frac{1}{2} \sum_{n=1}^{N}\left\{t_{n}-w_{0}-\sum_{j=1}^{M-1} w_{j} \phi_{j}\left(\mathbf{x}_{n}\right)\right\}^{2}$<br>
  $\displaystyle w_{0}=\bar{t}-\sum_{j=1}^{M-1} w_{j} \bar{\phi}_{j}$<br>
  $\displaystyle \bar{t}=\frac{1}{N} \sum_{n=1}^{N} t_{n}, \quad \bar{\phi}_{j}=\frac{1}{N} \sum_{n=1}^{N} \phi_{j}\left(\mathbf{x}_{n}\right)$<br>
  <br>
  $\displaystyle \frac{1}{\beta_{\mathrm{ML}}}=\frac{1}{N} \sum_{n=1}^{N}\left\{t_{n}-\mathbf{w}_{\mathrm{ML}}^{T} \phi\left(\mathbf{x}_{n}\right)\right\}^{2}$

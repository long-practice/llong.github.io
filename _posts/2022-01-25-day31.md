...
- 따라서 범함수를 최소화 시키면 됨<br>
  $\begin{cases}L_{kj} = 1 \quad (\mbox{if} k = j) \\ L_{kj} = 0 \quad (\mbox{if} k \ne j) \end{cases}$ 이므로 다음이 성립
  
- $\hat{C}(x) = \mbox{argmin} \displaystyle\sum_{k = 1}^{K}L_{kj}P(C_k \mid x) = \mbox{argmin} \sum_{k = 1}^{K}P(C_k \mid x) - P(C_j \mid x) = \mbox{argmin} 1 - P(C_j \mid x)$
...
- $ \displaystyle F\[y\] = \mathbb{E}\[L\] = \int_{\mathcal{R}} \int{\mathcal{X}} \\{y(t) - t\\}^2 P(x, t)dxdt = \int_{\mathcal{X}} \( \int_{\mathcal{R}}\{y(t) - t\}^2P(t|x)dt \)p(x)dx$에서

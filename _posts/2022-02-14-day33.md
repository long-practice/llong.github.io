...
- 관찰 데이터에 대해 우도함수를 표현하면 다음과 같음<br>
  $\displaystyle\ln p(\mathbf{X} \mid \boldsymbol{\mu}, \Sigma)=-\frac{N D}{2} \ln (2 \pi)-\frac{N}{2} \ln |\Sigma|-\frac{1}{2} \sum_{n=1}^{N}(\mathbf{x}\_{n}-\boldsymbol{\mu}\right)^{T} \Sigma^{-1}\left(\mathbf{x}\_{n}-\boldsymbol{\mu})$<br>
<br>

...
- 위의 식을 일반 방정식(normal equation)이라고 하며, 파라미터 추정 방식이 업데이트 방식이 아닌 일반 방정식의 풀이로 구해짐<br>
  $\Phi=\begin{array}{cccc}\left(\phi_{0}(x_{1}) & \phi_{1}(x_{1}) & \cdots & \phi_{M-1}(x_{1}) \\\\\\ \phi_{0}(x_{2}) & \phi_{1}(x_{2}) & \cdots & \phi_{M-1}(x_{2}) \\\\\\ \vdots & \vdots & \ddots & \vdots \\\\\\ \phi_{0}(x_{N}) & \phi_{1}(x_{N}) & \cdots & \phi_{M-1}(x_{N})\right)\end{array}$<br>
